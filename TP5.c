...
void procesarImagen(unsigned char * imagenIn, unsigned char * mensajeOut, unsigned int tamanio, unsigned char k)
{
	__asm {
        ...
        mov dl, 1  ; Guardo el numero 1 para hacer la operacion de bit shifting
        mov cl, [ebp+20]  ; Muevo a k a al, agregando ceros al final para tener el tamaño correcto
        shl dl, cl   ; Al 1 se le hace bit shifting por k unidades. k = al
        dec dl     ; Ya se hizo (1 << k). Ahora solo queda hacerle -1 a esto.
        and [edi], dl   ; Uso el operador AND entre imagenIn[i] y ((1 << k) - 1). Ahora, ecx es byteActual

		push ebx    ; push i para la funcion ingresar bits
		push [ebp+20]    ; push k para la funcion ingresar bits
		push [edi]    ; push byteActual para la funcion ingresar bits
...
void insert_bits(char *message, unsigned char bits, int bits_to_recover, int component_num) {
    ...
    __asm {
        // Calculate start_bit = component_num * bits_to_recover
        mov eax, component_num
        imul eax, bits_to_recover
        mov start_bit, eax
        ...
        for_loop:
            // Compare i with bits_to_recover
            mov ecx, bits_to_recover
            cmp i, ecx
            jge end_for
            
            // Calculate current_bit = (bits >> (bits_to_recover - 1 - i)) & 1
            mov ecx, bits_to_recover
            sub ecx, 1
            sub ecx, i
            mov bl, bits
            ...
            // Calculate message_byte = (start_bit + i) / 8
            mov eax, start_bit
            add eax, i
            cdq
            mov ecx, 8
            ...
            // Calculate bit_in_byte = 7 - ((start_bit + i) % 8)
            mov eax, start_bit
            add eax, i
            cdq
            mov ecx, 8
            ...
            mov cl, bit_in_byte
            shl bl, cl
            mov edi, message    ; Load message address into EDI
            add edi, eax        ; Add the byte offset
            or [edi], bl        ; Now perform the OR operation
